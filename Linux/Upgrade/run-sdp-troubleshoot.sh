...
yellowhammer() {
    if [ -d "${YHSERVERHOME}" ]; then 
        ...
        echo "[!] Unable to open ${YHSERVERHOME}"
...
redcanary() {
        if [ -d "${RCAGENTHOME}" ]; then 
            ...
            for f in ${RCAGENTHOME}/heapdump/*.hprof; do
                ...
                echo "[!] Unable to open ${RCAGENTHOME}"
                echo "[#] Seems like RedCanary-Agent isn't installed."
        fi

        if [ -d "${RCSERVERHOME}" ]; then
            ...
            echo "[!] Unable to get into ${YHWEBCLIENTHOME}"
            echo "[#] Seems like RedCanary-Server isn't installed."
        fi
