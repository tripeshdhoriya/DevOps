...
$Service_Name = "YellowHammer-WebClient" #service name.
...
$UPGRADE_BUNDLE_PATH = "$curdir\yellowhammer-webclient-upgrade.zip" #Upgrade zip filepath.
...
$Backup_File_Name = "yellowhammer-webclient_150.zip" #backup file name
$Old_Backup_Name = "$Backup_Directory\yellowhammer-webclient_109.zip" #old backup file name to delete.
$Restore_File_Name = "$Service_Name._old.zip"
$extract_foldername = "yellowhammer-webclient" #Foldername under zip file
    ...
    if ($?) {
        ...
        if ($?) {
            ...
            if ($?) {
                ...
                #7z.exe x $Tools_Directory\yellowhammer-webclient\webapp.zip > $null
                Set-Location $Tools_Directory\$extract_foldername
                Remove-Item -r $APPDIR\webapp\
				Write-Output "$APPDIR"
                7z.exe x .\webapp.zip -oC:\SigmaStream\YellowHammer\YellowHammer-WebClient\yellowhammer-webclient
                #Copy i18n changes.
                # Set-Location 
                Copy-Item -r -Force $Tools_Directory\$extract_foldername\i18n $APPDIR
                ...
                Write-Warning "[!] Failed to cd into $Tools_Directory\yellowhammer-webclient"
            }
        }
        else {
