#!/bin/bash
echo -n "[#] Checking connectivity to https://download.postgresql.org (timeout 30 seconds)"
curl --connect-timeout 30 -s -k https://download.postgresql.org > /dev/null
...
if [ $? -eq 0 ]; then
    ...
    echo " [  FAILED  ]"
    echo "[!] Unable to install postgres without internet connectivity."
fi