...
yellowhammer() {
    if [ -d "${YHSERVERHOME}" ] && [ -d "${YHWEBCLIENTHOME}" ] ;then 
        ...
        #--Begin YellowHammer-WebClient collection
        ...
        if [ -d "${RCAGENTHOME}" ]  && [ -d "${RCSERVERHOME}" ]; then 
            ...
            for f in ${RCAGENTHOME}/heapdump/*.hprof; do
                if [ -e "$f" ]; then
                    ...
                    printf "\t [#] No heapdump was generated by RedCanary-Agent. \n"
                fi
                break
            done

            echo "[+] Collecting RedCanary-Server files"
            sudo -u sigmastream mkdir -p ${DESTDIR}/RedCanary/RedCanry-Server
            cd ${RCSERVERHOME} || exit 1
            sudo cp -r logs/ ${DESTDIR}/RedCanary/RedCanry-Server/
            sudo cp -r config/ ${DESTDIR}/RedCanary/RedCanry-Server/
            sudo cp -r db/ ${DESTDIR}/RedCanary/RedCanry-Server/
            ...
            for f in ${RCSERVERHOME}/heapdump/*.hprof; do
                if [ -e "$f" ]; then
                    ...
                    printf "\t [#] No heapdump was generated by RedCanary-Server. \n"
                fi
                break
            done
        ...
        if [ -d "${BCHOME}" ]; then 
            ...
            for f in ${BCHOME}/heapdump/*.hprof; do
                if [ -e "$f" ]; then
                    ...
                    printf "\t [#] No heapdump was generated by BlueCardinal. \n"
                fi
                break
            done
            echo "[+] Compressing final files"
