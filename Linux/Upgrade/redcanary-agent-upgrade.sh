...
RCAGENTDIR=/apps/rcagent
BACKUPDIR=${UPGRADEDIR}/Backup/RedCanary-Agent/
...
BUNDLENAME=rc-agent-2.0.0
OLD_BACKUP=${BACKUPDIR}/RedCanary-Agent_985.tar
BACKUPFILENAME=RedCanary-Agent_985.tar
ZIPFILENAME=redcanary-agent-upgrade.zip
#current release= 
...
function stopservice {
    ...
    if [ $? -eq 0 ]
        then 
            ...
            sudo service rcagent stop > /dev/null 2>&1
            sleep 5
            sudo kill -9 "$(pgrep -f "/apps/rcagent")" > /dev/null 2>&1
            sudo kill -9 "$(pgrep -f "/apps/rcagent")" > /dev/null 2>&1
    else    
...
if [ -z "$1" ]; then
    if [ -f ${ZIPFILENAME} ]; then
        ...
        if [ -d ${UPGRADEDIR}/redcanary-agent/ ]; then 
            ...
            cd ${UPGRADEDIR}/redcanary-agent/ || exit 3
            
            echo -n "[+] Upgrading RedCanary-Agent..."
            
            cd ${UPGRADEDIR}/redcanary-agent/ || exit 3

            #append config changes
            sudo cat config.properties >> ${RCAGENTDIR}/config/config.properties
            ###

            ##jar change
            sudo mv ${RCAGENTDIR}/rc-agent.jar ${RCAGENTDIR}/rc-agent-jar
            sudo cp rc-agent.jar ${RCAGENTDIR}
            ###

            ##
            sudo cp check-rp-fd.sh ${RCAGENTDIR}/bin/
            sudo cp rc-yellowhammer-monitor-2.2.1-jar-with-dependencies.jar ${RCAGENTDIR}/lib/
            sudo chmod +x ${RCAGENTDIR}/bin/check-rp-fd.sh ${RCAGENTDIR}/lib/rc-yellowhammer-monitor-2.2.1-jar-with-dependencies.jar
            sudo cp MONITOR-RP.mon ${RCAGENTDIR}/deploy/
            sudo chmod 0755 ${RCAGENTDIR}/deploy/MONITOR-RP.mon
            ##

            ##
            if [ -f ${RCAGENTDIR}/lib/rc-yellowhammer-monitor-2.2.0-jar-with-dependencies.jar ]; then
                sudo rm ${RCAGENTDIR}/lib/rc-yellowhammer-monitor-2.2.0-jar-with-dependencies.jar > /dev/null
            # sudo cp rc-yellowhammer-monitor-2.2.0-jar-with-dependencies.jar ${RCAGENTDIR}/lib/rc-yellowhammer-monitor-2.2.0-jar-with-dependencies.jar
            fi
            #
            ...
            sudo rm ${RCAGENTDIR}/logs/*.log* > /dev/null 2>&1
            sudo rm ${RCAGENTDIR}/heapdump/*.hprof > /dev/null 2>&1
            
            echo " [ $GREEN OK $NORMAL ]"
            sudo chown -R sigmastream:sigmastream /apps/rcagent/
            ###

            cd ${UPGRADEDIR} > /dev/null 2>&1 || exit 3
            sudo rm -rf redcanary-agent > /dev/null 2>&1

            # echo " [ $GREEN OK $NORMAL ]"

            #Starting services... [optional]
            echo "[+] Attempting to start RedCanary-Agent..."
            sudo service rcagent start
            # echo " [ $GREEN OK $NORMAL ]"
            sleep 3
            sudo chown -R sigmastream:sigmastream ${UPGRADEDIR}/
        else
            echo "[#] Couldn't find RedCanary-Agent upgrade files at ${UPGRADEDIR}"
        fi
    else
